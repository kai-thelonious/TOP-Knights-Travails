...
function knightsMoves(start, end) {
	...
	let visited = new Set(); // The set makes sure we can't add the start node again
	...
	while (queue.length > 0) {
		...
		// Explore neighbors
		let moves = getLegalMoves(current.position);
		for (let move of moves) {
			if (!visited.has(move.toString())) {
				visited.add(move.toString());
				queue.push(new Node(move, [...current.path, move]));
			}
		}
	}
}
